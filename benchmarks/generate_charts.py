...
def generate_throughput_chart(data: dict, output_path: Path):
    ...
    short_labels = ["Hello World", "JSON Object", "Path Params", "Model Valid.", "Custom Status"]
    ...
    for bar, val in zip(bars1, turboapi_values, strict=False):
    ...
    for bar, val in zip(bars2, fastapi_values, strict=False):
        height = bar.get_height()
        ax.annotate(
            f"{val:,}",
            xy=(bar.get_x() + bar.get_width() / 2, height),
    ...
    for i, (turbo, fast) in enumerate(zip(turboapi_values, fastapi_values, strict=False)):
        if fast > 0:
            speedup = turbo / fast
            ax.annotate(
    ...
    ax.set_title(
        "Throughput Comparison: TurboAPI vs FastAPI",
        fontweight="bold",
        color=COLORS["text"],
        pad=20,
    )
    ...
    plt.savefig(
        output_path, dpi=150, facecolor=COLORS["background"], edgecolor="none", bbox_inches="tight"
    )
    plt.close()
...
def generate_latency_chart(data: dict, output_path: Path):
    ...
    # Create grouped bars
    ax.bar(x - 1.5 * width, turboapi_avg, width, label="TurboAPI (avg)", color=COLORS["turboapi"])
    ax.bar(
        x - 0.5 * width, turboapi_p99, width, label="TurboAPI (p99)", color=COLORS["turboapi_light"]
    )
    ax.bar(x + 0.5 * width, fastapi_avg, width, label="FastAPI (avg)", color=COLORS["fastapi"])
    ax.bar(
        x + 1.5 * width, fastapi_p99, width, label="FastAPI (p99)", color=COLORS["fastapi_light"]
    )

    ax.set_ylabel("Latency (ms)", fontweight="bold", color=COLORS["text"])
    ax.set_title(
        "Latency Comparison: TurboAPI vs FastAPI", fontweight="bold", color=COLORS["text"], pad=20
    )
    ax.set_xticks(x)
...
def generate_speedup_chart(data: dict, output_path: Path):
    ...
    speedups = [
        t / f if f > 0 else 0 for t, f in zip(turboapi_values, fastapi_values, strict=False)
    ]
    ...
    bars = ax.barh(y_pos, speedups, color=colors, height=0.6, edgecolor="white", linewidth=0.5)
    ...
    ax.set_title("TurboAPI Speedup vs FastAPI", fontweight="bold", color=COLORS["text"], pad=20)
    ax.set_xlim(0, max(speedups) * 1.3)

    # Add average speedup
    avg_speedup = sum(speedups) / len(speedups)
    ax.axvline(x=avg_speedup, color=COLORS["turboapi"], linestyle="-", linewidth=2, alpha=0.5)
    ax.text(
        avg_speedup + 0.1,
        -0.5,
        f"Average: {avg_speedup:.1f}x",
        color=COLORS["turboapi"],
        fontsize=11,
        fontweight="bold",
    )
...
def generate_architecture_diagram(output_path: Path):
    ...
    for i, feat in enumerate(features):
        ax.text(2 + (i % 2) * 4, 3.1 - (i // 2) * 0.5, f"• {feat}", fontsize=9, color="#4CAF50")

    # Add arrows
    arrow_props = {"arrowstyle": "->", "color": "#666", "lw": 1.5}
    for y in [6.6, 5.4, 4.2]:
        ax.annotate("", xy=(5, y), xytext=(5, y + 0.4), arrowprops=arrow_props)
